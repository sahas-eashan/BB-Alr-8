    ...
    def detect_color_box(self, frame):
        ...
        for contour in contours:
            if cv2.contourArea(contour) > 500:  # Adjust area threshold as needed
                ...
                cv2.putText(frame, f"{self.target_color.capitalize()} Box", (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
        ...
        if not color_detected:
            if time.time() - self.last_detection_time > self.timeout:
                self.direction = "Stop"
                cv2.putText(frame, "No target detected - Stopped", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
...
def main():
    color = input("Enter the color of the cube to follow (blue, red, green): ").strip().lower()
    while color not in ["blue", "red", "green"]:
        print("Invalid color choice. Choose from blue, red, or green.")
        color = input("Enter the color of the cube to follow (blue, red, green): ").strip().lower()
